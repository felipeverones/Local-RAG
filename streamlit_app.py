...
st.title("Inserir Novos Documentos")

uploaded_file = st.file_uploader("Escolha um arquivo CSV ou PDF", type=["csv", "pdf"])

if uploaded_file is not None:
    nome_arquivo = uploaded_file.name
    if nome_arquivo.endswith('.csv'):
        documentos = ler_csv(uploaded_file)
        inseridos, existentes = inserir_documentos(documentos, nome_arquivo)
    elif nome_arquivo.endswith('.pdf'):
        documentos = ler_pdf(uploaded_file)
        for i, doc in enumerate(documentos):
            inseridos, existentes = inserir_documentos([doc], f"{nome_arquivo}_chunk_{i}")
            total_inseridos += inseridos
            total_existentes += existentes
    else:
        st.error("Tipo de arquivo não suportado. Por favor, carregue um arquivo CSV ou PDF.")

    st.write(f"Arquivo {nome_arquivo}: {inseridos} documentos inseridos, {existentes} já existentes.")
